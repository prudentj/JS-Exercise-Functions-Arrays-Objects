...
*/
function getLastCarInfo(ven) {
  return ("This is a "+ ven[ven.length-1].car_make+ " "+ven[ven.length-1].car_model);
}
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inv, d_id) {
...
*/
function getOlderCars(inv , max_y) {
  const hold=[];
  for (var i=0; i < inv.length; i++){
    if(inv[i].car_year <= max_y) {
      hold.push(inv[i]);
    }
  }
  return hold;
 ...
 * in the same order as they appear in the original inventory.
*/
function getGermanCars(inv , max_y) {
  const hold=[];
  for (var i=0; i < inv.length; i++){
